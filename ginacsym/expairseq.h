...
public:
    ...
    const numeric & get_overall_coeff() const { return ex_to<numeric>(overall_coeff); }
...
class make_flat_inserter
{
	public:
		...
		ex handle_factor(const ex &x, const ex &coeff)
		{
			...
			if (is_a<numeric>(coeff) && coeff.is_equal(ginacsym::numeric(1)))
				dummies_of_factor = get_all_dummy_indices_safely(x);
			else if (is_a<numeric>(coeff) && coeff.is_equal(ginacsym::numeric(2)))
