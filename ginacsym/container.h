/** @file container.h
 *
 *  Wrapper template for making ginacsym classes out of STL containers. */
...
protected:
	...
	virtual void printseq(const print_context & c, char openbracket, char delim,
                          ...
                          unsigned upper_precedence = 0, bool isLatex=false) const;
...
template <template <class T, class = std::allocator<T>> class C>
void container<C>::do_print_latex(const print_latex & c, unsigned level) const
{
    // always print brackets around seq, ignore upper_precedence
    printseq(c, get_open_delim(), ',', get_close_delim(), precedence(), precedence()+1,true);
}
...
void container<C>::printseq(const print_context & c, char openbracket, char delim,
                            char closebracket, unsigned this_precedence,
                            unsigned upper_precedence, bool isLatex) const
{
    if(!isLatex){
